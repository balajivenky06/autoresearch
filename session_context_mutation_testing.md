...
All figures live under `plots_mutation/` (gitignored — regenerated by
the scripts noted below). Three groups: the main mutation testing
results, the Pynguin tool comparison, and the human evaluation.

### Main mutation results
| File | What it shows | Generated by |
|---|---|---|
| `kill_rate_heatmap.png` | 4×4 method × LLM heatmap, mean kill rate, annotated with values + n | `plot_mutation_heatmap.py` |
| `kill_rate_boundary_heatmap.png` | Same grid for boundary kill rate (the metric where IC vs Plain LLM is significant) | `plot_mutation_heatmap.py` |
| `kill_rate_combined_heatmap.png` | 1×5 mosaic: overall + 4 per-operator heatmaps | `plot_mutation_heatmap.py` |
| `kill_rate_by_method.png`, `kill_rate_by_operator.png` | Bar charts (legacy from `mutation_testing.py`) | `mutation_testing.py` |
| `mutation_rank_correlation*.png`, `mutation_rank_stability.png` | Cross-model Spearman ρ heatmaps + rank-stability lines | `analyze_mutation_generalizability.py` |
| `mutation_kill_rate_by_model.png` | Method × model grouped bars | `analyze_mutation_generalizability.py` |
| `noise_vs_kill_scatter.png` | 3-panel scatter of RAG-quality metrics vs kill rate | `noise_vs_kill.py` |

### Pynguin tool comparison ★
| File | What it shows | Generated by |
|---|---|---|
| `pynguin_vs_llm_kill_rate.png` | 5-bar mean kill rate with error bars: Pynguin + 4 LLM methods averaged across models | `plot_pynguin_comparison.py` |
| `pynguin_vs_llm_per_operator.png` | 5×5 grouped bars: 5 operators × 5 generators, highlights Pynguin's comparison-mutator gap (0.33 vs IC 0.96) | `plot_pynguin_comparison.py` |

### Human evaluation ★
| File | What it shows | Generated by |
|---|---|---|
| `human_eval_method_ranking.png` | 4 methods × 3 dimensions grouped bars, mean rating across 3 annotators. IC ranks highest on every dimension. | `plot_human_eval.py` |
| `human_eval_annotator_bias.png` | Per-annotator mean rating per dimension; visualises the SaeshwaranA scale-usage bias documented in §Limitations | `plot_human_eval.py` |
| `human_eval_correlations.png` | Per-sample scatter of mean human rating vs mutation kill rate, coloured by method, with Pearson r in each panel title | `plot_human_eval.py` |

### Regenerating all figures from scratch

```bash
python3 mutation_testing.py --results-only         # base bar charts
python3 plot_mutation_heatmap.py                   # 4×4 heatmaps
python3 analyze_mutation_generalizability.py       # Spearman ρ + rank stability
python3 noise_vs_kill.py                            # RAG quality vs kill scatter
python3 plot_pynguin_comparison.py                  # Pynguin vs LLM bars
python3 plot_human_eval.py                          # human-eval figures
```

All scripts read from `results_mutation.tsv`,
`human_eval_annotations/`, and `human_eval_pairs.meta.csv`. Nothing
requires GPU or Ollama at this point — figures are reproducible from
the persisted TSV + annotation CSVs alone.
